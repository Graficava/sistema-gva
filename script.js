...
const STATUSES=["Orçamento","Na fila de produção","Em produção","Acabamento","Pronto para Retirada","Entregue","Cancelado / Estorno"];
...
async function enviarPedido(imprimir = false, isOrcamento = false) {
    ...
    const saldo = total - pago; let statusInicial = "Em produção"; if (isOrcamento) statusInicial = "Orçamento"; else if (saldo > 0) statusInicial = "Na fila de produção";
...
async function confirmarRecebimentoSaldo() {
    ...
    const novoStatus = (novoSaldo === 0 && (p.status === 'Na fila de produção' || p.status === 'Orçamento')) ? 'Em produção' : p.status;
