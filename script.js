        ...
        grade.innerHTML += `
            <div class="zap-card">
                ...
                <div class="zap-info">
                    ...
                    <p style="font-size:20px; font-weight:bold; color:var(--gva-azul); margin:10px 0;">R$ ${p.preco.toFixed(2)}</p>
...
// ADMIN GESTÃO (EDITAR/EXCLUIR TUDO)
function renderizarListaAdmin() {
    ...
    div.innerHTML = `<table class="wp-table"><thead><tr><th>SKU</th><th>Nome</th><th>Ações</th></tr></thead><tbody>` + 
        bancoDeDados.map(p => `<tr><td>${p.sku || '--'}</td><td>${p.nome}</td><td><button onclick="editarProduto('${p.id}')">Editar</button><button onclick="excluirItem('catalogo', '${p.id}')" style="color:red; margin-left:10px;">X</button></td></tr>`).join('') + `</tbody></table>`;
}
...
function renderizarListaCategorias() {
    ...
    div.innerHTML = `<table class="wp-table"><thead><tr><th>Categoria</th><th>Ações</th></tr></thead><tbody>` + 
        categorias.map(c => `<tr><td>${c.nome}</td><td><button onclick="editarCategoria('${c.id}')">Editar</button><button onclick="excluirItem('categorias', '${c.id}')" style="color:red; margin-left:10px;">X</button></td></tr>`).join('') + `</tbody></table>`;
}
...
function renderizarListaAcabamentos() {
    ...
    div.innerHTML = `<table class="wp-table"><thead><tr><th>Acabamento</th><th>Ações</th></tr></thead><tbody>` + 
        acabamentos.map(a => `<tr><td>${a.nome}</td><td><button onclick="editarAcabamento('${a.id}')">Editar</button><button onclick="excluirItem('acabamentos', '${a.id}')" style="color:red; margin-left:10px;">X</button></td></tr>`).join('') + `</tbody></table>`;
}
...
function atualizarProducao() {
    ...
    div.innerHTML = pedidosGVA.map((p, idx) => `
        <div class="gva-card">
            ...
            <button class="btn-gva" style="font-size:11px; padding:5px 15px;" onclick="imprimirCupom(${idx})">Nota</button>
            <button class="btn-gva" style="background:#25D366; font-size:11px; padding:5px 15px;" onclick="enviarWA(${idx})"><i class="fab fa-whatsapp"></i></button></div>
        </div>`).join('');
}
...
// (Funções auxiliares: Salvar, Lancamento, Financeiro, etc)
