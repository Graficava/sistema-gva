...
function enviarWhatsApp(idPedido, tipo, objPedido = null) {
    ...
    let telefone = "";
    if (p.clienteId && p.clienteId !== "Consumidor Final") { const cli = bdClientes.find(c => c.id === p.clienteId); if (cli && cli.telefone) telefone = cli.telefone.replace(/\D/g, ''); }
...
async function salvarProduto() {
    ...
    try {
        ...
        alert("Produto salvo!"); limparFormProd(); mudarSubAba('sub-prod', document.querySelectorAll('.sub-aba-btn')[1]);
...
function editProd(id) {
    ...
    ajustarCamposProduto(); atualizarListaAcabamentosProduto(p.acabamentos ||[]); mudarSubAba('sub-prod', document.querySelectorAll('.sub-aba-btn')[1]);
}
...
function calcularPrecoAoVivo() {
    ...
    if (regra === 'm2') {
        const l = parseFloat(document.getElementById('w2pLargura')?.value) || 0, a = parseFloat(document.getElementById('w2pAltura')?.value) || 0; qtd = parseInt(document.getElementById('w2pQtd')?.value) || 1;
        const menorLado = Math.min(l, a);
        if (p && p.larguraMax > 0 && menorLado > p.larguraMax) { erroMax.classList.remove('hidden'); bloqueado = true; } else erroMax.classList.add('hidden');
        if (!bloqueado && p && p.larguraBobina > 0 && menorLado > p.larguraBobina) avisoBobina.classList.remove('hidden'); else avisoBobina.classList.add('hidden');
...
async function enviarPedido(imprimir = false, isOrcamento = false) {
    ...
    const idCli = document.getElementById('cartClienteId').value, nomeCliInput = document.getElementById('cartClienteInput').value, total = parseFloat(document.getElementById('totalCarrinho').innerText.replace("R$ ",""));
    ...
    const pedido = {
        clienteId: idCli || "Consumidor Final", clienteNome: idCli ? bdClientes.find(x => x.id === idCli).nome : (nomeCliInput || "Consumidor Final"),
        itens: carrinho, total: total, desconto: desconto, taxaPagto: taxaPagto, frete: frete, formaPagamento: formaPagto, valorPago: pago, saldoDevedor: saldo, data: new Date(), status: statusInicial, arquivado: false, pagamentos: pago > 0 ?[{ data: new Date(), valor: pago, forma: formaPagto }] :[]
    };
    ...
    carrinho =[]; document.getElementById('cartValorPago').value = 0; document.getElementById('cartDesconto').value = 0; document.getElementById('cartFreteValor').value = 0; document.getElementById('cartPagamento').value = 'Pix'; toggleOpcoesPagamento(); renderCarrinho();
}
...
// (Funções de CRUD de Categoria, Acabamento, Cliente e Usuário mantidas idênticas e compactadas para garantir espaço)
