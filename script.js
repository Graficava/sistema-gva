    ...
    if (user) {
        try {
            ...
            if (doc.exists) {
                ...
                // Se não existir, define como admin (ideal para você no primeiro acesso)
            ...
            // Fallback de segurança: se houver falha de rede ou regras, libera pra não travar a tela
            usuarioAtual = { nome: user.email.split('@')[0], email: user.email, role: "admin" };
        }
...
function cadastrar() {
    ...
    if (!e || !s) {
        msgErro.innerText = "Preencha e-mail e senha para criar.";
        msgErro.classList.remove('hidden');
        return;
    }
    
    auth.createUserWithEmailAndPassword(e, s).catch(err => {
        msgErro.innerText = "Erro ao criar: " + err.message;
        msgErro.classList.remove('hidden');
    });
}
...
function imprimirOrcamento(idPedido, objPedido) {
    ...
    let html = `
        ...
        <style>
            body { font-family: 'Segoe UI', sans-serif; margin: 0; padding: 40px; color: #334155; }
            ...
            th { background: #f1f5f9; text-align: left; padding: 12px; font-size: 12px; text-transform: uppercase; }
            ...
            .total-line.grand-total { font-size: 22px; font-weight: bold; border-top: 2px solid #cbd5e1; padding-top: 15px; margin-top: 15px; }
        </style>
        </head><body>
        <div class="header">
            <div><img src="https://i.postimg.cc/1RCc58qN/gva-br-1-ERP-26.png" class="logo" alt="GVA Gráfica" /></div>
            <div class="company-info"><strong>Gráfica Venom Arts LTDA</strong><br/>CNPJ: 17.184.159/0001-06<br/>WhatsApp: 21 99993-0190</div>
        </div>
        <div class="info-section">
            <div class="info-block"><div class="info-label">Orçamento preparado para:</div><div class="info-value">${p.clienteNome}</div></div>
            <div class="info-block text-right"><div class="info-label">Nº do Orçamento</div><div class="info-value">#${idPedido.substring(0,6).toUpperCase()}</div></div>
        </div>
        <table>
            <thead><tr><th>Item / Descrição</th><th class="text-right">Qtd</th><th class="text-right">Total</th></tr></thead>
            <tbody>
                ${p.itens.map(i => `<tr><td><strong>${i.nome}</strong><br/><span style="color:#64748b; font-size:12px;">${i.desc.replace(/\|/g, '<br/>')}</span></td><td class="text-right font-bold">${i.qtdCarrinho}</td><td class="text-right font-bold">R$ ${i.valor.toFixed(2)}</td></tr>`).join('')}
            </tbody>
        </table>
        <div class="totals">
            <div class="total-line grand-total"><span>Total Final:</span><span>R$ ${p.total.toFixed(2)}</span></div>
        </div>
        <div style="text-align: center; margin-top: 40px; font-weight: bold; color: #b45309;">
            Este orçamento é válido até ${dataValidade.toLocaleDateString('pt-BR')} (7 dias).
        </div>
...
function renderCat() { 
    ...
    if(acabCat) acabCat.innerHTML = (catSelect ? catSelect.innerHTML : ""); 
}
...
function verHistoricoCliente(idCli) { 
    ...
    corpo.innerHTML = pedidosCli.length === 0 ? "<p class='text-center text-slate-400 py-10'>Nenhum pedido.</p>" : pedidosCli.map(p => `
        <div class="bg-slate-50 p-4 rounded border border-slate-100">
            <div class="flex justify-between font-bold text-indigo-900 mb-2"><span>${p.data.toDate().toLocaleDateString('pt-BR')}</span><span>R$ ${p.total.toFixed(2)}</span></div>
            <div class="text-xs text-slate-500 mb-3">${p.itens.map(i => `• ${i.qtdCarrinho}x ${i.nome}`).join('<br/>')}</div>
